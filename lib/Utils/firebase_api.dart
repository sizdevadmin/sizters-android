...
class FirebaseApi {
      ...
      Navigator.push(
          ...
          MaterialPageRoute(
              builder: (context) => ChatInside(
                    lenderId: message.data["from_user"].toString(),
                    product: "",
                    order: "",
                  )));
    } else if (message.data["event"].toString() == "order_received") {
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => const AllOrderRequest()));
    } else if (message.data["event"].toString() == "order_reviewed") {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  RentDetails(productId: message.data["order"].toString())));
    } else if (message.data["event"].toString() == "product_reviewed") {
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  MyListing(fromListing: false, initialIndex: 0)));
  ...
  Future initLocalNotifications(BuildContext context) async {
    ...
    const android = AndroidInitializationSettings('@drawable/notiicon');
